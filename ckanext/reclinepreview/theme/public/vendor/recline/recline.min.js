...
return true;});},_remove:function(docs){var self=this;if(!(docs instanceof Array))docs=[docs];_.each(docs,function(doc){for(key in self.features._layers){if(self.features._layers[key].feature.properties.cid==doc.cid){self.features.removeLayer(self.features._layers[key]);}}});},_parseCoordinateString:function(coord){if(typeof(coord)!='string'||!coord){return(parseFloat(coord));}
var dms=coord.split(/[^\.\d\w]+/);var deg=0;var m=0;var toDeg=[1,60,3600];var i;for(i=0;i<dms.length;++i){if(isNaN(parseFloat(dms[i]))){continue;}
deg+=parseFloat(dms[i])/toDeg[m];m+=1;}
if(coord.match(/[SW]/)){deg=-1*deg;}
return(deg);},_getGeometryFromRecord:function(doc){if(this.state.get('geomField')){var value=doc.get(this.state.get('geomField'));if(typeof(value)==='string'){try{value=$.parseJSON(value);}catch(e){}}
if(typeof(value)==='string'){value=value.replace('(','').replace(')','');var parts=value.split(',');var lat=this._parseCoordinateString(parts[0]);var lon=this._parseCoordinateString(parts[1]);if(!isNaN(lon)&&!isNaN(parseFloat(lat))){return{"type":"Point","coordinates":[lon,lat]};}else{return null;}}else if(value&&_.isArray(value)){return{"type":"Point","coordinates":[value[0],value[1]]};}else if(value&&value.lat){return{"type":"Point","coordinates":[value.lon||value.lng,value.lat]};}
return value;}else if(this.state.get('lonField')&&this.state.get('latField')){var lon=doc.get(this.state.get('lonField'));var lat=doc.get(this.state.get('latField'));if(isNaN(parseFloat(lon))&&isNaN(parseFloat(lat))){console.log(this._parseCoordinateString(lon));}
lon=this._parseCoordinateString(lon);lat=this._parseCoordinateString(lat);if(!isNaN(parseFloat(lon))&&!isNaN(parseFloat(lat))){return{type:'Point',coordinates:[lon,lat]};}}
